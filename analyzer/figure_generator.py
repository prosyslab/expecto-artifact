...
RQ1_THRESHOLD_CAPTION = (
    r"Fig. 8. The quality of specifications generated by each tool under "
    r"different soundness threshold values $X$."
)
RQ2_FIGURE_CAPTION = (
    "Fig. 9. Comparison of the effectiveness of top-down specification "
    "synthesis and tree search in Expecto."
)
RQ3_FIGURE_CAPTION = (
    "Fig. 10. Comparison of the impact of test cases in SpecValidation."
)
RQ4_TABLE_CAPTION = "Table 2. Performance on the Defects4J benchmark"
...
_HUMANEVAL_FUNCTION_DOCSTRING_PATTERN = re.compile(
...
def _extract_humaneval_docstring_description(solution: Any) -> str:
    if not isinstance(solution, str) or not solution.strip():
        return ""

    match = _HUMANEVAL_FUNCTION_DOCSTRING_PATTERN.search(solution)
    if match is not None:
        return _normalize_nl_description(match.group("doc"))
    ...
    for node in module.body:
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if docstring:
                return _normalize_nl_description(docstring)
            for stmt in node.body:
                if not isinstance(stmt, ast.Expr):
                    continue
                value = stmt.value
                if isinstance(value, ast.Constant) and isinstance(value.value, str):
                    return _normalize_nl_description(value.value)
...
def _load_humaneval_nl2_run_descriptions(
    run_dir: Path,
    sample_ids: Iterable[str],
) -> dict[str, str]:
    remaining = {str(sample_id) for sample_id in sample_ids if str(sample_id).strip()}
    ...
    descriptions: dict[str, str] = {}
    jsonl_paths = sorted(run_dir.rglob("preprocessed_samples.jsonl"))
    for jsonl_path in jsonl_paths:
        with jsonl_path.open("r", encoding="utf-8") as handle:
            for line in handle:
                stripped = line.strip()
                ...
                row = json.loads(stripped)
                sample_id = _normalize_exported_sample_id(
                    "humaneval_plus", row.get("task_id", row.get("id", ""))
                )
                if sample_id not in remaining:
                    continue
                description = _extract_humaneval_docstring_description(
                    row.get("solution")
                )
                if not description:
                    continue
...
def _attach_nl_descriptions(
    ...
    *,
    ...
    if benchmark in {"apps", "humaneval_plus"}:
        ...
        if benchmark == "humaneval_plus" and variant.startswith("nl2_"):
            descriptions.update(_load_humaneval_nl2_run_descriptions(run_dir, missing_ids))
            still_missing = [
                sample_id for sample_id in missing_ids if sample_id not in descriptions
            ]
        if still_missing:
            descriptions.update(
                _load_evalplus_nl_descriptions(
                    benchmark, still_missing, run_dir=run_dir
                )
            )
...
def _build_table_pdf_document(table_tex: str) -> str:
    ...
    return "\n".join(
        [
            ...
            r"\newcommand{\apps}{APPS}",
            r"\newcommand{\humanevalplus}{HumanEval+}",
            r"\begin{document}",
...
def get_tex_table(inputs: list[AggregatedResult], caption: str, label: str) -> str:
    ...
    benchmarks = sorted(df["benchmark"].unique(), key=_benchmark_sort_key)
    ...
    for i, benchmark in enumerate(benchmarks):
        ...
        bench_name = _format_benchmark_tex(benchmark)
...
def get_main_comparison_tex_table(
    inputs: list[AggregatedResult], caption: str, label: str
) -> str:
    df = get_dataframe(inputs)
    benchmarks = sorted(df["benchmark"].unique(), key=_benchmark_sort_key)
    preferred_exp_order = ("Expecto", "nl2postcond (Base)", "nl2postcond (Simple)")
    exp_names_present = list(df["exp_name"].unique())
    exp_names = [
        name for name in preferred_exp_order if name in exp_names_present
    ] + sorted(name for name in exp_names_present if name not in preferred_exp_order)

    metrics = [
        ("sound_and_complete", r"\textbf{S\&C}"),
        ("sound_only", r"\textbf{S}"),
        ("complete_only", r"\textbf{C}"),
        ("wrong", r"\textbf{W}"),
    ]
    header_labels = {
        "Expecto": r"\textsc{Expecto}",
        "nl2postcond (Base)": r"\textsc{Base}",
        "nl2postcond (Simple)": r"\textsc{Simple}",
    }

    col_format = f"@{{}}lc{'r' * len(exp_names)}@{{}}"
    exp_headers = " & ".join(
        header_labels.get(name, _format_experiment_label(name)) for name in exp_names
    )
    ...
    lines.extend(
        [
            ...
            r"\toprule",
            f"\\textbf{{Benchmark}} & \\textbf{{Result}} & {exp_headers} \\\\",
            r"\midrule",
        ]
    )
    ...
    for i, benchmark in enumerate(benchmarks):
        ...
        for j, (metric_key, metric_label) in enumerate(metrics):
            row_parts = [
                f"\\multirow{{{len(metrics)}}}{{*}}{{{bench_name}}}" if j == 0 else "",
                metric_label,
            ]
            ...
            for exp_name in exp_names:
                ...
                values_series = cast(pd.Series, exp_data[metric_key])
                values = values_series.tolist()
                row_parts.append(str(int(values[0])) if values else "-")
...
def _add_figure_caption(
    fig: plt.Figure, caption_text: str | None, *, y: float = 0.99
) -> None:
    if not caption_text:
        return
    fig.text(
...
def draw_ablation_subplots(
    ...
    if len(ordered_benchmarks) == 1:
        ...
        if suptitle:
            fig.suptitle(suptitle, y=0.98, fontsize=CAPTION_FONT_SIZE)
        fig.tight_layout()
    ...
    if suptitle:
        fig.suptitle(suptitle, y=0.97, fontsize=CAPTION_FONT_SIZE)
    Path(output_file).parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_file, bbox_inches="tight")
    plt.close(fig)
...
def draw_rq1_fig(json_path: str, output_path: str):
    ...
    caption = RQ1_TABLE_CAPTION
    label = "tab:rq1-performance"
    tex_table = get_main_comparison_tex_table(datas, caption, label)
    ...
    if threshold_plot_data:
        ...
        draw_threshold_plot(
            ...
            caption_text=RQ1_THRESHOLD_CAPTION,
        )
...
def draw_rq4_fig(config_path: str, output_path: str):
    ...
    tex_table = get_main_comparison_tex_table(
        datas,
        caption=RQ4_TABLE_CAPTION,
